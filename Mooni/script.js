...
// Variables
var mooni = document.getElementById("mooni");
...
// Activate mooni and set timeout for awaiting a command.
function activatemooni() {
	userMadeDecision = false;
	mooni.classList.remove("inactive");
	mooni.classList.add("active");
	setTimeout(function() {
		if (!userMadeDecision) {
			mooni.classList.remove("active");
			mooni.classList.add("inactive");
			setTimeout(function() {
				mooni.classList.remove("inactive");
			}, 750);
		}
